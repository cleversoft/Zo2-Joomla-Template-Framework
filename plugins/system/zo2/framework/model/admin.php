    ...
    class Zo2ModelAdmin {
            ...
            if ($this->_isAuthorized()) {
                if (JFolder::exists(ZO2PATH_CACHE)) {
                    if (JFolder::delete(ZO2PATH_CACHE)) {
                        $this->_ajax->addMessage(JText::_('ZO2_ADMIN_MODEL_CACHE_CLEARED'), 'success');
                    } else {
                        $this->_ajax->addMessage(JText::_('ZO2_ADMIN_MODEL_CLEAR_FAILED'), 'error');
                    }
                } else {
                    $this->_ajax->addMessage(JText::_('ZO2_ADMIN_MODEL_CACHED'), 'info');
                }
            }
            $this->_ajax->response();
        }
        ...
        public function buildAssets() {
            if ($this->_isAuthorized()) {
                ...
                $assets->buildAssets();
                $this->_ajax->addMessage(JText::_('ZO2_ADMIN_MODEL_BUILD_ASSETS_SUCCESS'), 'success');
            }
            $this->_ajax->response();
        }
